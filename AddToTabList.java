...
public class AddToTabList {
    public void addToTab(Player p, String name, int id, Boolean registerTeam, String TeamName, String playername) {
        ...
        if(registerTeam) {
            ...
            ScoreboardTeam ScoreboardTeam = new ScoreboardTeam(nmsServer.getScoreboard(), TeamName);
            connection.sendPacket(new PacketPlayOutScoreboardTeam(ScoreboardTeam, 0));

            ArrayList<String> playerToAdd = new ArrayList<>();
            playerToAdd.add(entityPlayer.getUniqueIDString());
            connection.sendPacket(new PacketPlayOutScoreboardTeam(ScoreboardTeam, playerToAdd, 3));
        }else {
